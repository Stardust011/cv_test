...
{
	Point Predict_Point;
	Predict_Point.x = (double)(Start.x - Cente.x)*(double)cos(angle*PI / 180) - (double)(Start.y - Cente.y)*(double)sin(angle*PI / 180) + Cente.x;
	Predict_Point.y = (double)(Start.y - Cente.y)*(double)cos(angle*PI / 180) + (double)(Start.x - Cente.x)*(double)sin(angle*PI / 180) + Cente.y;
...
void Ligth::Circle_Center(Point2f POINT1, Point2f POINT2, double &R, Point2f &P_Center)
{
	...
	{
		const double c1 = (POINT2.x*POINT2.x - POINT1.x * POINT1.x + POINT2.y * POINT2.y - POINT1.y * POINT1.y) / (2 * (POINT2.x - POINT1.x));
...
{
	double Redious;
	Redious = sqrtf((Point_F.x - Point_S.x)*(Point_F.x - Point_S.x) + (Point_F.y - Point_S.y)*(Point_F.y - Point_S.y));
	return Redious;
}
void Ligth::Dafu_Out(Mat &image, bool justy, Rect &roi, double &M_N, Point2f &Get_Point)
{
	...
	for (int first = 0; first<contours0.size(); first++)
	{
		...
		if (angle == 1 && P_Center[math].x != 0 && P_Center[math].y != 0 && contourArea(contours0[first]) >= 1000 && contourArea(contours0[first]) < 2500)
		{
