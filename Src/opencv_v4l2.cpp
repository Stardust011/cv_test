...
V4L2Capture *set_v4l(char *video_path) {
    string videoDev = video_path;
...
Mat VideoCapture_v4l(V4L2Capture *vcap) {
    ...
    vcap->getFrame((void **) &yuv422frame, (size_t *) &yuvframeSize);
    Mat cvmat(IMAGEHEIGHT, IMAGEWIDTH, CV_8UC3, (void *) yuv422frame);        //CV_8UC3
    img = imdecode(&cvmat, IMREAD_COLOR);
    if (!img) {
